...
# proveedor real. Usa la misma logica que tests/test_local.py (brain + memory),
# sin pasar por providers/. Se registra SOLO si ENVIRONMENT != "production": no
# tiene sentido dejar un chat libre con el agente expuesto en un deploy real,
# donde cualquiera con la URL podria hablar con el y gastar creditos de la API.
if ENVIRONMENT != "production":
    ...
    async def test_chat(payload: MensajeTest):
        """
        Genera una respuesta para el chat de prueba, igual que tests/test_local.py
        pero por HTTP. No pasa por providers/: no hay WhatsApp real de por medio.
        """
